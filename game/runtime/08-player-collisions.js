...
function startDash(){
  ...
  else{ let best=null,bestScore=1e9; const baseAng=Math.atan2(dy,dx); for(const e of enemies){ if(!e.active||e.state==='spawn') continue; const ex=e.x-player.x,ey=e.y-player.y,dist=Math.hypot(ex,ey); if(dist<34||dist>210) continue; const diff=Math.abs(((Math.atan2(ey,ex)-baseAng+Math.PI*3)%(Math.PI*2))-Math.PI); if(diff<0.38){ const score=diff*180+dist*0.25; if(score<bestScore){ bestScore=score; best={x:ex,y:ey}; } } } if(best){ dx=best.x; dy=best.y; } }
